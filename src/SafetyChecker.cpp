...
namespace crossroads {
    ...
    SafetyChecker::SafetyChecker() : SafetyChecker(makeDefaultIntersectionConfig()) {
    }

    SafetyChecker::SafetyChecker(const IntersectionConfig& config)
        : intersection_config(config), config_valid(validateConfig(config)) {
    }
    ...
    bool SafetyChecker::validateConfig(const IntersectionConfig& config) const {
        ...
        auto find_lane_for_id = [&](LaneId lane_id, ApproachId& approach, const LaneConfig*& lane_cfg) {
            for (const auto& entry : config.approaches) {
                auto found = std::find_if(entry.lanes.begin(), entry.lanes.end(), [lane_id](const LaneConfig& lane) {
        ...
        for (const auto& group : config.signal_groups) {
            ...
            for (LaneId lane : group.controlled_lanes) {
                ...
                for (MovementType movement : group.green_movements) {
                    const bool lane_supports_movement =
                        std::find(lane_cfg->allowed_movements.begin(), lane_cfg->allowed_movements.end(), movement) !=
            ...
            for (size_t i = 0; i < group_movements.size(); ++i) {
                for (size_t j = i + 1; j < group_movements.size(); ++j) {
                    if (hasMovementConflict(group_movements[i].from,
                                            group_movements[i].movement,
                                            group_movements[j].from,
                                            group_movements[j].movement)) {
                        return false;
                    }
                }
            }
    ...
    bool SafetyChecker::hasMovementConflict(ApproachId from_a,
                                            MovementType move_a,
                                            ApproachId from_b,
                                            MovementType move_b) const {
        ...
        bool opposite_pair = ((from_a == ApproachId::North && from_b == ApproachId::South) ||
    ...
    bool SafetyChecker::areSignalGroupsConflictFree(const std::vector<SignalGroupId>& active_group_ids) const {
        ...
        for (SignalGroupId group_id : active_group_ids) {
            auto group_it = std::find_if(intersection_config.signal_groups.begin(),
                                         ...
                                         [group_id](const SignalGroupConfig& g) { return g.id == group_id; });
        ...
        for (size_t i = 0; i < active_movements.size(); ++i) {
            for (size_t j = i + 1; j < active_movements.size(); ++j) {
                if (hasMovementConflict(active_movements[i].from,
                                        active_movements[i].movement,
                                        active_movements[j].from,
                                        active_movements[j].movement)) {
                    return false;
                }
            }
        }
    ...
    bool SafetyChecker::checkTurningLightSafety(const IntersectionState& state) const {
        ...
        auto turning_pair_conflicts =
            [&](ApproachId from_a, MovementType move_a, ApproachId from_b, MovementType move_b) {
                auto collect_target_lanes = [&](ApproachId from, MovementType movement) {
                    ...
                    for (const auto& connection : intersection_config.lane_connections) {
                        ...
                        targets.push_back(
                            laneIdFor(connection.to_approach, static_cast<std::size_t>(connection.to_lane_index)));
                    }
        ...
        if (state.turnSouthEast == LightState::Green && is_active(state.west) &&
            turning_pair_conflicts(ApproachId::South, MovementType::Right, ApproachId::West, MovementType::Straight))
            return false;

        // turnNorthWest cannot be green if East is active
        if (state.turnNorthWest == LightState::Green && is_active(state.east) &&
            turning_pair_conflicts(ApproachId::North, MovementType::Right, ApproachId::East, MovementType::Straight))
            return false;

        // turnWestSouth cannot be green if North is active
        if (state.turnWestSouth == LightState::Green && is_active(state.north) &&
            turning_pair_conflicts(ApproachId::West, MovementType::Right, ApproachId::North, MovementType::Straight))
            return false;

        // turnEastNorth cannot be green if South is active
        if (state.turnEastNorth == LightState::Green && is_active(state.south) &&
            turning_pair_conflicts(ApproachId::East, MovementType::Right, ApproachId::South, MovementType::Straight))
            return false;
        ...
        if (state.turnNorthEast == LightState::Green && is_active(state.south) &&
            hasMovementConflict(ApproachId::North, MovementType::Left, ApproachId::South, MovementType::Straight))
            return false;
        if (state.turnSouthWest == LightState::Green && is_active(state.north) &&
            hasMovementConflict(ApproachId::South, MovementType::Left, ApproachId::North, MovementType::Straight))
            return false;
        if (state.turnEastSouth == LightState::Green && is_active(state.west) &&
            hasMovementConflict(ApproachId::East, MovementType::Left, ApproachId::West, MovementType::Straight))
            return false;
        if (state.turnWestNorth == LightState::Green && is_active(state.east) &&
            hasMovementConflict(ApproachId::West, MovementType::Left, ApproachId::East, MovementType::Straight))
            return false;

        return true;
    }

    bool SafetyChecker::isValidTransition(const IntersectionState& prev,
                                          const IntersectionState& next,
                                          double dt_seconds) const {
        return checkPerLightTransitions(prev, next) && checkOrangeTiming(prev, next, dt_seconds) && isSafe(next) &&
               checkCrossingLightSafety(prev, next) && checkTurningLightTransitions(next);
    }
    ...
    bool SafetyChecker::checkPerLightTransitions(const IntersectionState& prev, const IntersectionState& next) const {
        ...
        return valid_for_light(prev.north, next.north) && valid_for_light(prev.east, next.east) &&
               valid_for_light(prev.south, next.south) && valid_for_light(prev.west, next.west) &&
    ...
    bool SafetyChecker::checkOrangeTiming(const IntersectionState& prev,
                                          const IntersectionState& next,
                                          double dt_seconds) const {
        ...
        return check_orange_duration(prev.north, next.north) && check_orange_duration(prev.east, next.east) &&
               check_orange_duration(prev.south, next.south) && check_orange_duration(prev.west, next.west) &&
