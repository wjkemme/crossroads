...
TEST_CASE("Vehicle crossing duration scales with density", "[vehicle]") {
    Vehicle v(1, Direction::North, 0.0);
    REQUIRE(v.getCrossingDuration(0) == Catch::Approx(1.87));
    REQUIRE(v.getCrossingDuration(5) == Catch::Approx(2.55));
    REQUIRE(v.getCrossingDuration(10) == Catch::Approx(3.23));
}
...
TEST_CASE("Mixed straight-left lane keeps dedicated left signal red under opposing conflict",
          "[engine][regression][left-turn]") {
...
TEST_CASE("Left-only lane without traffic light never enters crossing for free or existing destination lanes",
          "[engine][regression][left-turn]") {
    class FixedConflictController : public ITrafficLightController {
       public:
        void tick(double) override {
...
TEST_CASE("Left-only lane with traffic light uses red dedicated left signal when opposing approach is active",
          "[engine][regression][left-turn]") {
...
TEST_CASE("Dedicated left lane gets explicit left green while main can stay green",
          "[engine][regression][left-signal]") {
...
TEST_CASE("Exclusive dedicated right turn can actuate without waiting full opposing red cycle",
          "[engine][regression][right-turn]") {
...
TEST_CASE("SafetyChecker movement conflict rules cover main corridors", "[safety][config]") {
    ...
    REQUIRE(checker.hasMovementConflict(
        ApproachId::North, MovementType::Straight, ApproachId::East, MovementType::Straight));

    REQUIRE_FALSE(checker.hasMovementConflict(
        ApproachId::North, MovementType::Straight, ApproachId::South, MovementType::Straight));

    REQUIRE(checker.hasMovementConflict(ApproachId::North, MovementType::Left, ApproachId::South, MovementType::Left));
}
...
TEST_CASE("SimulatorEngine keeps green active for at least three seconds", "[engine][timing][green-min]") {
    ...
    for (int i = 0; i < 80; ++i) {
        ...
        if (first_green_time >= 0.0 && snapshot.lights.north != LightState::Green &&
...
TEST_CASE("SimulatorEngine scheduler lists only configured approach-movement routes", "[engine][scheduler][routes]") {
    ...
    config.approaches[0].lanes = {{900, "N-left", {MovementType::Left}, true, true, true}};
    config.approaches[2].lanes = {{920, "S-straight", {MovementType::Straight}, true, true, true}};
    config.approaches[1].lanes = {{910, "E-straight", {MovementType::Straight}, true, true, true}};
    config.approaches[3].lanes = {{930, "W-straight", {MovementType::Straight}, true, true, true}};
    for (auto& approach : config.approaches) {
        approach.to_lane_count = 1;
    }
...
TEST_CASE("SimulatorEngine effective lights enforce orange step and minimum orange duration",
          "[engine][transition][effective]") {
    SimulatorEngine engine(makeDefaultIntersectionConfig(), 8.0, 10.0, 10.0);
    engine.start();

    auto light_values = [](const nlohmann::json& lights) {
        return std::array<std::string, 12>{lights.value("north", std::string{"red"}),
...
TEST_CASE("SimulatorEngine prevents starvation for South to North straight under competing north-left demand",
          "[engine][fairness][starvation]") {
    class NSMainGreenController : public ITrafficLightController {
       public:
        void tick(double) override {
        }
    ...
    for (int i = 0; i < 700; ++i) {
        ...
        if (saw_north_left_demand && south_straight_waiting &&
...
TEST_CASE("SimulatorEngine prevents starvation for North to East left under competing south-right demand",
          "[engine][fairness][starvation]") {
    class AllRedController : public ITrafficLightController {
       public:
        void tick(double) override {
    ...
    for (int i = 0; i < 900; ++i) {
        ...
        if (south_right_waiting && north_left_waiting &&
            doc["lights"].value("turnSouthEast", std::string{"red"}) == "red") {
            saw_conflicting_right_paused = true;
        }

        if (north_left_waiting && doc["lights"].value("turnNorthEast", std::string{"red"}) == "green") {
...
TEST_CASE("TrafficGenerator assigns movement intent from lane config", "[traffic][config][movement]") {
    ...
    for (const auto& vehicle : north_queue) {
        REQUIRE((vehicle.movement == MovementType::Straight || vehicle.movement == MovementType::Right ||
