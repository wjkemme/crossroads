...
namespace crossroads {
    ...
    struct Vehicle {
        ...
        Vehicle(uint32_t vid, Direction lane, double arrival)
            ...
            , exit_time(-1.0)
            , current_speed(0.0)
            , position_in_lane(0.0)
            , turning(false)
            , queue_index(0)
            , lane_id(0)
            , movement(MovementType::Straight)
            , destination_approach(ApproachId::North)
            , destination_lane_index(0)
            , destination_lane_id(0)
            , lane_change_allowed(true) {
        }

        bool isWaiting() const {
            return crossing_time < 0.0;
        }
        bool isCrossing() const {
            return crossing_time >= 0.0 && exit_time < 0.0;
        }
        bool hasCrossed() const {
            return exit_time >= 0.0;
        }
